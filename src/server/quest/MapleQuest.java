...
public class MapleQuest {
		...
		for (MapleQuestRequirement r : startReqs.values()) {
			...
			if (!r.check(c, npcid)) {
				if (r.getType().getType() == MapleQuestRequirementType.INTERVAL.getType()) {
					c.message("This quest will become available again in approximately "
	...
	public boolean canComplete(MapleCharacter c, Integer npcid) {
		...
		for (MapleQuestRequirement r : completeReqs.values()) {
			if (r == null || !r.check(c, npcid)) {
				if (r.getType() == MapleQuestRequirementType.MESO) { // TODO:
																		// find
																		// a way
																		// to
																		// tell
	...
	public boolean forceComplete(MapleCharacter c, int npc) {
		...
		c.getClient().getSession().write(MaplePacketCreator.showSpecialEffect(9)); // Quest
																					// completion
		c.getMap().broadcastMessage(c, MaplePacketCreator.showForeignEffect(c.getId(), 9), false); // use
																									// 9
																									// instead
																									// of
																									// 12
	...
	private MapleQuestRequirement getRequirement(MapleQuestRequirementType type, MapleData data) {
		...
		case INFO_NUMBER:
			break;
		default:
			// FilePrinter.printError(FilePrinter.EXCEPTION_CAUGHT, "Unhandled
			// Requirement Type: " + type.toString() + " QuestID: " +
			// this.getId());
			break;
		}
		return ret;
	...
	private MapleQuestAction getAction(MapleQuestActionType type, MapleData data) {
		...
		default:
			// FilePrinter.printError(FilePrinter.EXCEPTION_CAUGHT, "Unhandled
			// Action Type: " + type.toString() + " QuestID: " + this.getId());
			break;
		}
		return ret;
	}
