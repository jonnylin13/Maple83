...
public class Server implements Runnable {
	 ...
	 * 
	 * World world = worlds.get(worldid); if (world != null) {
	...
	public void run() {
		...
		try {
			...
			// Set all accounts to loggedin = 0
		...
		tMan.start();
		tMan.register(tMan.purge(), ServerConstants.PURGING_INTERVAL);// Purging
																		// ftw...
		...
		// MapleItemInformationProvider.getInstance().getAllItems(); //unused,
		// rofl
		...
		timeToTake = System.currentTimeMillis();
		MapleQuest.loadAllQuests();
		System.out.println("Quest loaded in " + ((System.currentTimeMillis() - timeToTake) / 1000.0) + " seconds\r\n");

		try {
			Integer worldCount = Math.min(ServerConstants.WORLD_NAMES.length,
					Integer.parseInt(p.getProperty("worlds")));
			...
			System.out.println("Error in configuration.ini.");
			System.exit(0);
		}
		...
		/*
		 * this.sc = new Scanner(System.in); while (true) { if (!online) break;
	...
	public final Runnable shutdown(final boolean restart) {// no player should
															// be online when
															// trying to
															// shutdown!
		return new Runnable() {
			...
			public void run() {
				...
				try {
					 ...
					 * (w.getPlayerStorage().getAllCharacters().size() > 0) {
					 * try { Thread.sleep(1000); } catch (InterruptedException
					 * ie) { System.err.println("FUCK MY LIFE"); } } } for
					 * (Channel ch : getAllChannels()) { while
					 * (ch.getConnectedClients() > 0) { try {
