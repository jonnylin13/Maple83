...
public class MapleServerHandler extends IoHandlerAdapter {
	...
	private short lastRecvPacketId;
	...
	public void sessionOpened(IoSession session) {
		...
		if (channel > -1 && world > -1) {
			...
			FilePrinter.print(FilePrinter.SESSION, "IoSession with " + session.getRemoteAddress() + " opened on "
					+ sdf.format(Calendar.getInstance().getTime()), false);
		}
		...
		client.setSessionId(sessionId.getAndIncrement()); // Generates a
															// reasonable
															// session id.
	...
	public void messageReceived(IoSession session, Object message) {
		...
		if (this.lastRecvPacketId != 0 && ServerConstants.USE_DEBUG_SHOW_RCVD_PACKET
				&& this.lastRecvPacketId != packetId) {
				...
				FilePrinter.printError(FilePrinter.PACKET_HANDLER + packetHandler.getClass().getName() + ".txt", t,
						"Error for "
								+ (client.getPlayer() == null ? ""
										: "player ; " + client.getPlayer() + " on map ; "
